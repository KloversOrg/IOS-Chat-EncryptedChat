...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	[push setData:@{@"alert":message, @"sound":@"default", @"badge":@"Increment", @"content-available" : @"1"}];
	[push sendPushInBackgroundWithBlock:^(BOOL succeeded, NSError *error)
	{
		if (error != nil)
		{
			NSLog(@"SendPushNotification2 send error %@", error );
            
            		}
	}];
    
    [PFCloud callFunctionInBackground:@"push"
                       withParameters:@{@"message":text}
                                block:^(NSObject *ratings, NSError *error) {
                                    if (!error) {
                                        // ratings is 4.5
                                        NSLog(@"");
                                    }else{
